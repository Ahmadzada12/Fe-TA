        ...
        <table class="w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="donation in filteredDonations" :key="donation.id">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-900">{{ formatCurrency(donation.donationTarget) }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-900">{{ formatCurrency(donation.donationCollected || 0) }}</div>
                <div class="text-xs text-gray-500">
                  {{ Math.round((donation.donationCollected || 0) / donation.donationTarget * 100) }}%
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-900">
                  {{ formatDateRange(donation.donationStartDate, donation.donationFinishedDate) }}
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span :class="getStatusClass(donation.statusDonasi)" 
                      class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full">
                  {{ getStatusText(donation.statusDonasi) }}
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span v-if="donation.deletedAt" 
            ...
            <tr v-if="filteredDonations.length === 0">
              <td colspan="7" class="px-6 py-4 text-center text-gray-500">
                {{ donations.length === 0 ? 'Belum ada campaign' : 'Tidak ada campaign yang sesuai filter' }}
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <div v-if="showAddModal || showEditModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div class="relative top-10 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
        <div class="mt-3">
          ...
          <form @submit.prevent="showAddModal ? addDonation() : updateDonation()">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Target Donasi (Rp)</label>
                <input 
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Donasi Terkumpul (Rp)</label>
                <input 
            ...
            <div class="mb-6">
              <label class="block text-sm font-medium text-gray-700 mb-2">
                ...
                <span v-if="showEditModal" class="text-gray-500">(Opsional - kosongkan jika tidak ingin mengubah gambar)</span>
              </label>
              <input 
              ...
              <p class="text-xs text-gray-500 mt-1">
                ...
                <span v-if="showEditModal" class="block mt-1 text-blue-600">
                  Gambar saat ini akan tetap digunakan jika tidak ada file baru yang dipilih
                </span>
              </p>
            </div>
...
const editDonation = (donation: any) => {
  ...
  donationForm.value = {
    ...
    donationStartDate: donation.donationStartDate ? new Date(donation.donationStartDate).toISOString().slice(0, 16) : '',
    donationFinishedDate: donation.donationFinishedDate ? new Date(donation.donationFinishedDate).toISOString().slice(0, 16) : ''
  }
